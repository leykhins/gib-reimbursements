...
const groupedRequests = computed(() => {
  ...
  // Sort expenses within each group by status first (manager_approved at top), then by date
  Object.keys(groups).forEach(jobNumber => {
    groups[jobNumber].sort((a, b) => {
      // First prioritize manager_approved status
      if (a.status === 'manager_approved' && b.status !== 'manager_approved') {
...
// Get paginated job numbers with priority for those with manager_approved requests
const paginatedJobNumbers = computed(() => {
  // Sort job numbers to prioritize those with manager_approved requests
  const sortedJobNumbers = Object.keys(groupedRequests.value).sort((a, b) => {
    const aHasManagerApproved = groupedRequests.value[a].some(r => r.status === 'manager_approved');
    const bHasManagerApproved = groupedRequests.value[b].some(r => r.status === 'manager_approved');
    
    if (aHasManagerApproved && !bHasManagerApproved) {
      return -1; // a comes before b
    }
    if (!aHasManagerApproved && bHasManagerApproved) {
      return 1; // b comes before a
    ...
    // If tied on manager_approved status, sort by most recent expense date
...
// Update fetchReimbursementRequests to include employee information
...
// Update status class function to match pending page
