...
const fetchReimbursementRequests = async (month = null, year = null) => {
  try {
    ...
    if (month === null || (month === selectedMonth.value && year === selectedYear.value)) {
      ...
      const newClaims = (data || []).filter(claim => !existingIds.has(claim.id))
...
// Toggle category function with auto-expand of jobs
...
const fetchMonthlyStatusIndicators = async () => {
  try {
    ...
    // Fetch only date and status for pending claims in this year
    const { data, error } = await client
      .from('claims')
      .select('date, status')
      .eq('status', 'pending')
...
<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h1 class="text-xl font-bold">Pending Reimbursements</h1>
    </div>
