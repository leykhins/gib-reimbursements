...
// Get paginated job numbers but prioritize those with pending requests and newest first
const paginatedJobNumbers = computed(() => {
  // Sort job numbers based on priority: 
  // 1. Has pending requests
  // 2. Most recent request date
  const sortedJobNumbers = Object.keys(groupedRequests.value).sort((a, b) => {
    // Check for pending requests in each group
    const aHasPending = groupedRequests.value[a].some(req => req.status === 'pending')
    const bHasPending = groupedRequests.value[b].some(req => req.status === 'pending')
    
    // If one has pending and the other doesn't, prioritize the one with pending
    if (aHasPending && !bHasPending) return -1
    if (!aHasPending && bHasPending) return 1
    
    // If both have same pending status, sort by most recent request date
    const aLatestDate = new Date(groupedRequests.value[a][0].created_at)
    const bLatestDate = new Date(groupedRequests.value[b][0].created_at)
  ...
  const start = (currentPage.value - 1) * itemsPerPage
  const end = start + itemsPerPage
  return sortedJobNumbers.slice(start, end)
})
...
const fetchReimbursementRequests = async () => {
  try {
    ...
    // Then, fetch reimbursement requests for that department
    const { data, error: fetchError } = await client
      ...
      `)
      .eq('profiles.department', userData.department)
...
const applyFilters = () => {
  // First filter to only show admin_verified requests and exclude pending
  filteredRequests.value = reimbursementRequests.value.filter(request => {
    // Only include admin_verified requests
...
// Modified reject function for manager to reject admin_verified requests
...
<template>
  <div class="space-y-6">
    <div class="flex justify-between items-center">
      <h1 class="text-2xl font-bold">Admin Verified Expenses</h1>
    </div>
    ...
    <Card>
      <CardHeader>
        <CardTitle>Admin Verified Reimbursement Requests</CardTitle>
        <CardDescription>
          {{ filteredRequests.length }} admin verified requests found
        </CardDescription>
      </CardHeader>
                  ...
                  <tr v-for="request in groupedRequests[jobNumber]" :key="request.id" class="border-t hover:bg-muted/50">
                    ...
                    <td class="px-4 py-3">
                      <div class="flex space-x-2">
                        ...
                        <!-- Manager Approval Button (only for admin_verified requests) -->
                        <Button 
                          v-if="request.status === 'admin_verified'"
                          variant="outline" 
                          size="sm"
                        ...
                        <!-- Manager Reject Button (only for admin_verified requests) -->
                        <Button 
                          v-if="request.status === 'admin_verified'"
                          variant="outline" 
                          size="sm"
