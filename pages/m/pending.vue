...
const selectedCategorySubcategories = computed(() => {
  ...
  return category?.category_subcategory_mapping?.map(mapping => mapping.subcategory) || []
})
...
const toggleEmployeeSelection = (employeeId, checked) => {
  ...
  // Collect all verified requests for this employee
...
const isEmployeeFullySelected = (employeeId) => {
  ...
  Object.values(employee.categories).forEach(category => {
    Object.values(category.jobGroups).forEach(jobGroup => {
      jobGroup.requests.forEach(request => {
        if (request.status === 'verified') {
          verifiedRequests.push(request.id)
        }
      })
    })
  })
  
  return verifiedRequests.length > 0 && 
         verifiedRequests.every(id => selectedRequests.value.has(id))
}
...
// Update the monthlyClaimStatus to track both verified and approved claims
    ...
    <Dialog v-model:open="showVerifyModal">
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>
            Are you sure you want to approve {{ verifyingRequests.length }} request{{ verifyingRequests.length > 1 ? 's' : '' }}?
